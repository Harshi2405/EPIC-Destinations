...
function showMovies(rs, cs) {
    ...
    Object.keys(moviesData).forEach(_id => {
        ...
        <img height="124" width="91" class="mr-3 shadow hide-on-md" src="assets/${moviesData[_id].Image}">
        <div class="media-body flex-wrap">
        <h3 class="font-bold mb-0 hover:underline cursor-pointer" onclick="javascript: window.location = 'https://www.imdb.com/title/${_id}';">${moviesData[_id].Destination} <small class="text-gray">${moviesData[_id].State}</small></h3>
        ...
        template+=`
        <div class="card p-2 px-4 flex  mb-2">
        <img height="325" width="200" class="mr-3 shadow hide-on-md" src="assets/images/${moviesData[_id].Image}">
        <div class="media-body flex-wrap">
        <h3 class="font-bold mb-0"><b>${moviesData[_id].Destination}</b> <small>${moviesData[_id].State}</sdamall></h3>
        <h4 class="text-black ">${moviesData[_id].Description}</h4>
        <h4 class="text-black "><b>HOTELS:</b>${moviesData[_id].Hotels}</h4>
        <h4 class="text-black "><b>FOOD:</b>${moviesData[_id].Food}</h4>
        <h4 class="text-black "><b>AVGEXPENSES:</b>${moviesData[_id].AvgExp}</h4>
        <h4><font class="bg-primary p-2 px-4 text-white"><i class="fal fa-star"></i> <span id="${_id}_rating_id">${rv}</span></font></h4>
        <input type="range" class="range-slider" min="1" max="100" data-id="${_id}" onchange="javascript: saveRating(this)" oninput="javascript: inputRating(this);" value="${rv}" id="myRange">
        </div></div>`;
    });
...
function saveRating(el) {
    ...
    user.Reviews[id] = { "Title": moviesData[id].Destination, "Rating": r };
...
function recMovies() {
    ...
    // Get recommendation according to site.
        reviews["u0"] = user;
        movies = getRecommendation("u0", reviews, simScoreName[simScore], 5);
    ...
    for (let i = 0; i < limit; i++) {
       ...
       /* template += `<div class="card p-2 mb-2">
        ...
        template += `<div class="card p-2  mb-2">
        <img height="285" width="100%" class="mr-3 shadow hide-on-md" src="assets/images/${movies[i].Image}">
        <div class="media-body flex-wrap">
        <h3 class="font-bold mb-0">${movies[i].Destination} <small>${movies[i].State}</sdamall></h3>
        <h4 class="text-black">${movies[i].Description}</h4> 
        <h4 class="text-black"><b>HOTELS:</b>${movies[i].Hotels}</h4>
        <h4 class="text-black"><b>FOOD:</b>${movies[i].Food}</h4>
        <h4 class="text-black"><b>AVGEXPENSES:</b>${movies[i].AvgExp}</h4> 
        </div></div>`;
    }
...
function getRecommendation(selectedId, data, correlationType, k) {
    ...
    Object.keys(items).forEach(_id => {
        ...
        //result.push({ "Rating": items[_id].Rating, "Title": moviesData[_id].Destination, "id": _id });
        result.push({ "Rating": items[_id].Rating, "Destination": moviesData[_id].Destination, "State": moviesData[_id].State, "Hotels": moviesData[_id].Hotels, "Food": moviesData[_id].Food, "Description": moviesData[_id].Description,"AvgExp": moviesData[_id].AvgExp,"id": _id , "Image":moviesData[_id].Image});
    });
