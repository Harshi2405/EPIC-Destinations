...
function showMovies(rs, cs) {
    ...
    Object.keys(Data).forEach(_id => {
        ...
        <img height="325" width="200" class="mr-3 shadow hide-on-md" src="assets/images/${Data[_id].Image}">
        <div class="media-body flex-wrap">
        <h3 class="font-bold mb-0"><b>${Data[_id].Destination}</b> <small>${Data[_id].State}</sdamall></h3>
        <h4 class="text-black ">${Data[_id].Description}</h4>
        <h4 class="text-black "><b>HOTELS:</b>${Data[_id].Hotels}</h4>
        <h4 class="text-black "><b>FOOD:</b>${Data[_id].Food}</h4>
        <h4 class="text-black "><b>AVGEXPENSES:</b>${Data[_id].AvgExp}</h4>
        <h4><font class="bg-primary p-2 px-4 text-white"><i class="fal fa-star"></i> <span id="${_id}_rating_id">${rv}</span></font></h4>
        <input type="range" class="range-slider" min="1" max="100" data-id="${_id}" onchange="javascript: saveRating(this)" oninput="javascript: inputRating(this);" value="${rv}" id="myRange">
        </div></div>`;
    });
...
function saveRating(el) {
    ...
    user.Reviews[id] = { "Title": Data[id].Destination, "Rating": r };
...
function getRecommendation(selectedId, data, correlationType, k) {
    ...
    Object.keys(items).forEach(_id => {
        ...
        result.push({ "Rating": items[_id].Rating, "Destination": Data[_id].Destination, "State": Data[_id].State, "Hotels": Data[_id].Hotels, "Food": Data[_id].Food, "Description": Data[_id].Description,"AvgExp": Data[_id].AvgExp,"id": _id , "Image":Data[_id].Image});
    });
